...
def recursive_minimax_strategy(game: Any) -> Any:
    ...
    new_state = [current_state.make_move(m) for m in move_list]
    for state in new_state:
        move_score.append(recursive_helper(game, state, current_player))
    return move_list[move_score.index(max(move_score))]


def recursive_helper(game: Any, state: Any, player: str) -> int:
    """
    a helper for recursion
    """
    ...
    if game.is_over(current_state):
        ...
        return max([-1 * recursive_helper(game, s, player) for s in
                    [current_state.make_move(m) for m in
